ui <- navbarPage(
  ...
  tabPanel(
    ...
    tags$p("Emissions provided in the TRI are largely used here since it's the most consistent and complete data set.", 
           "In cases where multiple data sources were availlable for a given year, e.g., NEI, then the maximum emissions of NEI/EPA are used. "),
  ...
  tabPanel(
    ...
    tags$p(
      "Data used for the map and charts are shown below. Note that cancer risks for years when EPA did not conduct its", 
      "assessment, i.e., 2015, 2016, 2018 - present, are estimated using a simple weighted approach. A more complex ", 
      "(and hopefully more accurate) estimate using inverse-distance weighting (IDW) approach is in the works. "
    ),
  ...
  tabPanel(
    ...
    tags$p("Updates on cancer risk will be made based on 2021 ethylene oxide emissions. ",
           "An inverse-distance weighted (IDW) approach to estimate cancer risks for all census tracts for years when EPA did not conduct", 
    ...
    tags$h4('Source'),
    tags$p('The code for this dashboard is available on', tags$a('Github', href='https://github.com/ehe-analytics/ethylene-oxide/tree/main/dashboard', target = '_blank'), '.')
  )
)